...
const float FOLLOW_THRESHOLD = 20.0f; // Seuil de distance à partir duquel la voiture commence à suivre la prochaine voie de circulation
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        // Suivi des voies de circulation
            ...
            if (road.direction) {
                ...
                if (distance < FOLLOW_THRESHOLD) {
                    ...
                    if (std::abs(car.position.x - road.start.x) > ROAD_WIDTH / 2) {
                        // La voiture n'est pas sur la bonne voie, modifiez l'angle de braquage pour qu'elle se dirige vers la voie de circulation
                        car.steer_angle = (car.position.x < road.start.x) ? -1.0f : 1.0f;
                    } else {
                        // La voiture est sur la bonne voie, réinitialisez l'angle de braquage à zéro
                        car.steer_angle = 0.0f;
                    }
                }
            } else {
                ...
                if (distance < FOLLOW_THRESHOLD) {
                    ...
                    if (std::abs(car.position.y - road.start.y) > ROAD_WIDTH / 2) {
                        // La voiture n'est pas sur la bonne voie, modifiez l'angle de braquage pour qu'elle se dirige vers la voie de circulation
                        car.steer_angle = (car.position.y < road.start.y) ? -1.0f : 1.0f;
                    } else {
                        // La voiture est sur la bonne voie, réinitialisez l'angle de braquage à zéro
                        car.steer_angle = 0.0f;
                    }
        ...
        // Mise à jour de la vitesse et de la position de la voiture
