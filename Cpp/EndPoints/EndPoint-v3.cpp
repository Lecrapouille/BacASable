...
public:
    ...
    , m_strategy(std::move(strategy))
    {
        ...
        // Configure the real endpoint with our internal buffers
...
public:
    ...
    // ------------------------------------------------------------------------
    // @brief Add an endpoint to the bus
    ...
    void addEndPoint(std::shared_ptr<EndPoint> endpoint)
    {
        ...
        // Lookup factory in registry based on endpoint's dynamic type
        auto it = m_registry.find(std::type_index(typeid(*endpoint)));
...
private:
    ...
    std::unordered_map<std::type_index, FakeFactory> m_registry;  // EndPoint type -> FakeEndPoint factory
    std::vector<std::shared_ptr<EndPoint>> m_endpoints;           // Collection of real endpoints
    std::vector<std::shared_ptr<FakeEndPoint>> m_fakes;           // Collection of fake endpoints
    FakeEndPoint::PrepareStrategy::Type m_strategy_type;          // Strategy type for all fakes
};
