...
Uses Gemini **function calling** (sometimes called “tool use”): you declare one
tool whose shape mirrors a Python function. The model either asks the user follow-up
questions in natural language or emits a structured **function_call** payload with
argument values extracted from the conversation.

Tutorial mental model:

1. You write a plain Python function.
2. ``@llm_fill`` attaches a wrapper that introspects ``inspect.signature``
   plus type hints.
3. The wrapper derives a Gemini ``FunctionDeclaration`` (name, description,
   JSON-like parameter schema).
4. In a terminal loop we send messages to Gemini; when the model issues a tool
   call with the right name, we read ``args`` and invoke your underlying function.

Requirements: ``pip install -r requirements.txt``  
Auth: ``GOOGLE_API_KEY`` or ``GEMINI_API_KEY`` (Google AI Studio).
"""
...
def llm_fill(
    system_prompt: str = (
        "You are a friendly reception assistant. Ask short polite questions "
...
#
# Gemini expects ``types.Type.STRING`` … not raw strings—the dict below resolves the
# hop from textual JSON Schema names to Google's enum literals.
# -----------------------------------------------------------------------------
